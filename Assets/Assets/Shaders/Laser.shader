Shader "Unlit/Laser"
{
    Properties
    {
        ...
        _LineLength("Line length", Float) = .5
        _MiddleSpeed("Middle speed", Float) = .5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                middle *= 0 + abs(sin(15 + i.uv.x * 15 + _Time.x * 235 * _MiddleSpeed));
                float edge = step(.5, centerness) - middle;
                edge *= 1 + abs(sin(i.uv.x * 15 + _Time.x * 135 * _MiddleSpeed));
